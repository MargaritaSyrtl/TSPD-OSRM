...
def generate_random_agent(waypoints):
    ...
    """
    first_node = waypoints[0]  # fix the first node
    remaining_nodes = list(waypoints[1:])  # exclude the first node
    random.shuffle(remaining_nodes)  # shuffle the rest of the nodes
    return tuple([first_node] + remaining_nodes)  # Combine fixed city with
...
def mutate_agent(agent_genome, max_mutations=3):
    ...
    for mutation in range(num_mutations):
        swap_index1 = random.randint(1, len(agent_genome) - 1)  # 1: exclude the first (fixed) node
        swap_index2 = swap_index1

        while swap_index1 == swap_index2:
            swap_index2 = random.randint(1, len(agent_genome) - 1)  # 1: exclude the first (fixed) node
...
def shuffle_mutation(agent_genome):
    ...
    start_index = random.randint(1, len(agent_genome) - 1)  # 1: exclude the first (fixed) node
    length = random.randint(2, 20)

    genome_subset = agent_genome[start_index: start_index + length]
    agent_genome = agent_genome[:start_index] + agent_genome[start_index + length:]

    insert_index = random.randint(1, len(agent_genome) + len(genome_subset) - 1)  # 1: ensure the first city is not moved
